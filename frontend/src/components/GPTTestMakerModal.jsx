...
function friendlyError(detail) {
  ...
  for (const [key, msg] of Object.entries(USER_ERRORS)) {
    if (lowerDetail.includes(key.toLowerCase())) return msg;
  }
          ...
          <>
            {step === 1 && (
              <div className="space-y-5">
                ...
                <div>
                  <label className="block text-[10px] font-bold mb-1 uppercase tracking-wider text-[#0A0A0A]/60">YouTube Video URL</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] font-bold mb-1 uppercase tracking-wider text-[#0A0A0A]/60">Number of Questions</label>
                    <select 
                  ...
                  <div>
                    <label className="block text-[10px] font-bold mb-1 uppercase tracking-wider text-[#0A0A0A]/60">Difficulty</label>
                    <select 
                      className="field-input"
                      value={difficulty}
                  ...
                  {showAdvanced && (
                    <div className="grid grid-cols-2 gap-4 mt-4 animate-in fade-in slide-in-from-top-1">
                      <div className="col-span-2">
                        <label className="block text-xs font-bold mb-1 text-[#0A0A0A]/70">Custom Title (Optional)</label>
                        <input 
                          type="text"
                          className="field-input"
                      ...
                      <div>
                        <label className="block text-xs font-bold mb-1 text-[#0A0A0A]/70">Passing Score (0-10000)</label>
                        <input 
                          type="number"
                          className="field-input"
                      ...
                      <div>
                        <label className="block text-xs font-bold mb-1 text-[#0A0A0A]/70">Duration (Min)</label>
                        <input 
                          type="number"
                          className="field-input"
              ...
              <div className="space-y-4">
                ...
                <div className="max-h-[350px] overflow-y-auto space-y-2 pr-1 custom-scrollbar">
                  {editableQuestions.map((q, qIdx) => (
                    <div key={qIdx} className="game-panel !bg-[#FFFFFF] !shadow-none !border-2 border-[#0A0A0A]">
                      <button 
                      ...
                      >
                        <div className="flex items-center gap-3 overflow-hidden">
                          ...
                          <span className="truncate text-sm text-[#0A0A0A] font-bold">{q.question_text}</span>
                        </div>
                        <div className="flex items-center gap-2 shrink-0">
                           <span className="text-[10px] font-mono font-bold text-[#0A0A0A]/40 uppercase">XP {q.weight * 100}</span>
                      ...
                      {expandedIdx === qIdx && (
                        <div className="p-3 pt-0 border-t-2 border-[#0A0A0A]/10 animate-in fade-in slide-in-from-top-1">
                          <div className="space-y-3 mt-3">
                            <div>
                              <label className="text-[10px] font-bold text-[#0A0A0A]/40 uppercase block mb-1">Question Text</label>
                              <textarea 
                                className="field-input"
                            ...
                            <div className="grid grid-cols-2 gap-3">
                              <div>
                                <label className="text-[10px] font-bold text-[#0A0A0A]/40 uppercase block mb-1">Difficulty Weight (1-5)</label>
                                <input 
                              ...
                              <div className="flex items-center gap-2 pt-4">
                                ...
                                />
                                <label htmlFor={`mult-${qIdx}`} className="text-xs font-bold text-[#0A0A0A]/70 uppercase">Allow Multiple</label>
                              </div>
                            </div>

                            <div className="space-y-1.5">
                              <label className="text-[10px] font-bold text-[#0A0A0A]/40 uppercase block">Options</label>
                              {q.options.map((opt, oIdx) => (
                                <div key={oIdx} className="flex items-center gap-2">
                                  ...
                                  <input 
                                    type="text" 
                                    className={`field-input !py-1.5 !text-xs ${opt.is_correct ? 'bg-[#00A95C]/10 border-[#00A95C]' : ''}`}
                ...
                <div className="flex gap-3 pt-2">
                  <button onClick={() => setStep(1)} className="flex-1 game-btn !bg-[#FFFFFF] py-4 flex items-center justify-center gap-2">
            ...
            {step === 3 && (
              <div className="space-y-6">
                <div className="game-panel p-5 bg-[#FFFFFF] relative overflow-hidden">
                  <div className="absolute top-0 right-0 w-32 h-32 bg-[#7C3AED]/5 -mr-16 -mt-16 rounded-full blur-3xl"></div>
                  
                  <div className="flex gap-4 relative z-10">
                    <img src={generatedData?.thumbnail_url} alt="thumbnail" className="w-[140px] h-[80px] object-cover border-2 border-[#0A0A0A]" />
                    <div className="flex-1">
                       <label className="text-[10px] font-bold text-[#0A0A0A]/40 uppercase block mb-1">Test Title</label>
                       <input 
                         type="text" 
                ...
                <div className="flex flex-col gap-3">
                  <button onClick={handleSave} className="w-full game-btn game-btn-blue py-5 flex items-center justify-center gap-2 text-lg">
                    <Check size={24} /> SAVE TO DINOQUEST
                  </button>
                  <button onClick={() => setStep(2)} className="w-full game-btn !bg-white py-4 flex items-center justify-center gap-2 font-bold opacity-60 hover:opacity-100 transition-opacity">
